...
select count(*) as "Total de Clientes com mais de 30 anos"
...
select MAX(extract(year from age(current_date, birth_date))) as "Idade Mais velho",
MIN(extract(year from age(current_date,birth_date))) as "Menor Idade"
from sales.customers
...
select brand, count(brand) as "Total das marcas" from sales.products
	group by brand;
...
select brand, model_year, count(*) from sales.products group by brand, model_year
order by brand , model_year

-- (Exercício 6) Conte quantos veículos de cada marca tem registrado na tabela sales.products
-- e mostre apenas as marcas que contém mais de 10 veículos registrados
select brand, count(*) from sales.products
