...
caminhoArquivoMovies = "s3://sprint07/raw/Local/CSV/Movies/16/10/2024/movies.csv"
caminhoDestino = f"s3://sprint07/trusted/Movies-CSV/{ano}/{mes}/{dia}/"
# Le o arquivo CSV do filmes usando Spark, definindo cabeçalho e demais informações
filmes_df = spark.read.option("delimiter", "|").csv(caminhoArquivoMovies, header=True, inferSchema=True)
...
filmesDrama_Romance.write.mode("overwrite").partitionBy("genero", "anoLancamento").parquet(caminhoDestino)
